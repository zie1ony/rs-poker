...
impl TournamentReplay {
    ...
    pub fn step_to_next_hand(&mut self) -> Result<Option<GameReplay>, ReplayError> {
        ...
        let game_replay = GameReplay::new(self.current_tournament_state.clone(), hand_actions.clone());
    ...
    pub fn get_hand_replay(&self, hand_index: usize) -> Result<GameReplay, ReplayError> {
        let hand_actions = self.replay_data.hands
            .get(hand_index)
        ...
        for i in 0..hand_index {
            ...
            temp_tournament_state = self.calculate_updated_tournament_state(&temp_tournament_state, &hand_replay);
        }
    ...
    fn update_tournament_state_from_hand(&mut self, hand_replay: &GameReplay) {
        self.current_tournament_state = self.calculate_updated_tournament_state(&self.current_tournament_state, hand_replay);
    }
    
    fn calculate_updated_tournament_state(&self, current_state: &GameState, hand_replay: &GameReplay) -> GameState {
...
impl TournamentHistorian {
    ...
    /// Call this when a hand ends to save the actions and prepare for the next hand
    pub fn finish_hand(&mut self) {
        if !self.current_hand_actions.is_empty() {
            self.tournament_data.add_hand(self.current_hand_actions.drain(..).collect());
        }
    }
...
impl ReplayTournamentBuilder {
    ...
    pub fn build_tournament_replay(self) -> Result<TournamentReplay, ReplayError> {
        let data = self.tournament_data.ok_or(ReplayError::MissingInitialGameState)?;
...
mod tests {
    ...
    use crate::arena::{
        ...
        action::{Action, GameStartPayload, DealStartingHandPayload},
    };
    ...
    fn test_tournament_replay_creation() {
        ...
        assert_eq!(tournament_replay.get_current_tournament_state().stacks, stacks);
        assert_eq!(tournament_replay.get_current_tournament_state().big_blind, 10.0);
        assert_eq!(tournament_replay.get_current_tournament_state().small_blind, 5.0);
    ...
    fn test_tournament_historian() {
        ...
        historian.record_action(1, &initial_state, action1.clone()).unwrap();
        historian.record_action(2, &initial_state, action2.clone()).unwrap();
        ...
        historian.record_action(3, &initial_state, action3.clone()).unwrap();
        historian.finish_hand();
