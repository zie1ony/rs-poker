use rs_poker::arena::{
    GameReplay, GameState, HoldemSimulationBuilder, 
...
fn main() {
    ...
    assert!(!original_board.is_empty(), "Board should have community cards");
    ...
    assert!(original_hands[0].count() >= 2, "Player 0 should have at least 2 cards");
    assert!(original_hands[1].count() >= 2, "Player 1 should have at least 2 cards");
    ...
    println!("   ✓ Player 0 hand (first 2 cards): {:?}", original_hands[0].iter().take(2).collect::<Vec<_>>());
    ...
    assert!(num_actions >= 6, "Should have at least deal + betting actions"); // Deal cards + some actions
    ...
    assert_eq!(replay.get_current_action_index(), 0, "Should start at action 0");
    assert!(replay.has_more_actions(), "Should have actions to replay");
    assert_eq!(replay.get_actions().len(), num_actions, "Should have all actions");
    assert_eq!(replay.get_current_state().stacks, initial_state.stacks, "Initial stacks should match");
    assert_eq!(replay.get_current_state().total_pot, 0.0, "Initial pot should be 0");
    ...
    while replay.has_more_actions() {
        ...
        step_count += 1;
        assert!(step_count <= num_actions, "Should not exceed recorded actions");
    }
    
    assert_eq!(step_count, num_actions, "Should step through all actions");
    assert!(!replay.has_more_actions(), "Should have no more actions after completion");
    assert_eq!(replay.get_current_action_index(), num_actions, "Should be at final action index");
    
    println!("   ✓ Successfully stepped through all {} actions", step_count);
    ...
    assert_eq!(replay_board, original_board, "Community cards must match exactly");
    assert_eq!(replay_hands.len(), original_hands.len(), "Number of hands must match");
    
    for (i, (replay_hand, original_hand)) in replay_hands.iter().zip(original_hands.iter()).enumerate() {
        assert_eq!(
            replay_hand.iter().collect::<Vec<_>>(), 
            original_hand.iter().collect::<Vec<_>>(),
            "Player {} hand cards must match exactly", i
        );
    }
    ...
    assert!(jump_result.is_ok(), "Should successfully jump to middle action");
    assert_eq!(replay.get_current_action_index(), mid_action, "Should be at requested action");
    ...
    assert!(early_result.is_ok(), "Should successfully jump to early action");
    assert_eq!(replay.get_current_action_index(), early_action, "Should be at early action");
    ...
    assert_eq!(replay.get_current_action_index(), num_actions, "Should be at end");
    
    // Verify cards are still exact after time travel
    assert_eq!(replay.get_current_state().board, original_board, "Board should still match after time travel");
    assert_eq!(
        replay.get_current_state().hands[0].iter().collect::<Vec<_>>(),
    ...
    assert!(invalid_jump.is_err(), "Should fail when jumping beyond available actions");
    
    println!("   ✓ Properly handles invalid action index");
    
    // Test reset functionality
    replay.step_to(0).unwrap();
    assert_eq!(replay.get_current_action_index(), 0, "Should be back at start");
    assert_eq!(replay.get_current_state().stacks, initial_state.stacks, "Should be back to initial stacks");
    assert_eq!(replay.get_current_state().total_pot, 0.0, "Should be back to initial pot");
    ...
    for i in 1..=std::cmp::min(5, num_actions) {
        ...
        assert_eq!(replay.get_current_action_index(), i, "Action index should match step");
        ...
        assert!(current_state.total_pot >= 0.0, "Pot should never be negative");
    }
