use rs_poker::arena::{
    GameState, TournamentReplayData, ReplayTournamentBuilder,
    competition::SingleTableTournamentBuilder,
    agent::{AllInAgentGenerator, FoldingAgentGenerator, AgentGenerator},
    action::Action,
};
...
fn main() {
    ...
    assert!(original_places.contains(&1), "Should have a winner (place 1)");
    ...
    assert_eq!(unique_places, vec![1, 2, 3, 4], "Places should be 1, 2, 3, 4");
    ...
    assert_eq!(tournament_replay.get_replay_data().num_hands(), 3, "Should have 3 hands");
    assert_eq!(tournament_replay.get_current_hand_index(), 0, "Should start at hand 0");
    assert!(tournament_replay.has_more_hands(), "Should have hands to replay");
    ...
    println!("   ✓ Number of hands to replay: {}", tournament_replay.get_replay_data().num_hands());
    ...
    if tournament_replay.has_more_hands() {
        ...
        match step_result.unwrap() {
            Some(_hand_replay) => {
                assert_eq!(tournament_replay.get_current_hand_index(), 1, "Should be at hand 1 now");
    ...
    assert_eq!(tournament_replay.get_current_hand_index(), 2, "Should be at hand 2");
    ...
    assert!(hand_0_replay.is_ok(), "Should successfully get hand 0 replay");
    assert_eq!(tournament_replay.get_current_hand_index(), 2, "Current index should still be 2");
    println!("   ✓ Successfully retrieved hand 0 without changing current state");
    
    // Test reset functionality
    tournament_replay.reset_to_start();
    assert_eq!(tournament_replay.get_current_hand_index(), 0, "Should be back at hand 0");
    assert_eq!(
    ...
    assert!(invalid_jump.is_err(), "Should fail when jumping to non-existent hand");
    ...
    assert!(invalid_hand.is_err(), "Should fail when getting non-existent hand");
    ...
    while tournament_replay.has_more_hands() {
        let step_result = tournament_replay.step_to_next_hand();
        assert!(step_result.is_ok(), "Should successfully step through all hands");
        hands_stepped += 1;
        assert!(hands_stepped <= 3, "Should not step through more than 3 hands");
    }
    assert_eq!(hands_stepped, 3, "Should have stepped through exactly 3 hands");
    assert!(!tournament_replay.has_more_hands(), "Should have no more hands after stepping through all");
    println!("   ✓ Successfully stepped through all {} hands", hands_stepped);
    ...
    assert!(replay_results.is_some(), "Results should be preserved in replay data");
    
    let replay_results = replay_results.unwrap();
    assert_eq!(replay_results.places(), original_places.as_slice(), "Places should match original results");
    assert_eq!(replay_results.rounds(), original_rounds, "Rounds should match original results");
