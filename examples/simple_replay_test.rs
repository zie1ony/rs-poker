...
fn main() {
    ...
    let game_state = GameState::new_starting(stacks.clone(), big_blind, small_blind, ante, dealer_idx);
    ...
    println!("✓ Created replay with {} actions", replay.get_actions().len());
    println!("✓ Starting step-by-step comparison...");
    
    // Compare initial states
    println!("\n🔍 Comparing initial states:");
    assert_eq!(replay.get_current_state().stacks, initial_state.stacks, "Initial stacks must match");
    assert_eq!(replay.get_current_state().total_pot, initial_state.total_pot, "Initial pot must match");
    assert_eq!(replay.get_current_state().board.len(), initial_state.board.len(), "Initial board length must match");
    ...
    while replay.has_more_actions() {
        ...
        println!("\n🎯 Step {}: Applying action {}", step_count + 1, action_index);
        println!("   Action: {:?}", current_action);
        
        // Record state before action
        let state_before = replay.get_current_state().clone();
        println!("   Before: pot={}, stacks={:?}, board_len={}", 
                 state_before.total_pot, state_before.stacks, state_before.board.len());
        ...
        match step_result {
            Ok(Some(_applied_action)) => {
                ...
                println!("   After:  pot={}, stacks={:?}, board_len={}", 
                         state_after.total_pot, state_after.stacks, state_after.board.len());
                ...
                match current_action {
                    Action::DealStartingHand(payload) => {
                        ...
                        assert!(state_after.hands[payload.idx].count() >= 1, 
                                "Player {} should have at least 1 card after deal", payload.idx);
                        println!("     ✓ Dealt card to player {}", payload.idx);
                    }
                    Action::DealCommunity(_) => {
                        // Community cards should be added to board
                        assert!(state_after.board.len() >= state_before.board.len(), 
                    ...
                    Action::PlayedAction(payload) => {
                        ...
                        if payload.final_pot > payload.starting_pot {
                            assert!(state_after.total_pot >= state_before.total_pot, 
                    ...
                    Action::ForcedBet(payload) => {
                        ...
                        assert!(state_after.stacks[payload.idx] <= state_before.stacks[payload.idx],
    ...
    println!("     Player 0 hand count: {}", final_sim_state.hands[0].count());
    ...
    println!("     Player 0 hand count: {}", final_replay_state.hands[0].count());
    
    // Critical comparisons
    assert_eq!(final_replay_state.board, final_sim_state.board, 
               "Final board must match exactly");
    assert_eq!(final_replay_state.total_pot, final_sim_state.total_pot, 
               "Final pot must match exactly");
    assert_eq!(final_replay_state.stacks, final_sim_state.stacks, 
               "Final stacks must match exactly");
    
    // Compare hands card by card
    for (i, (sim_hand, replay_hand)) in final_sim_state.hands.iter().zip(final_replay_state.hands.iter()).enumerate() {
